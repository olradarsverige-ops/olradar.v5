...
export default function LangPage(){
  ...
  const weekly = Object.entries(logsByUser).map(([user, dates])=>{
    ...
    return { user, count, xp: xpForLogs(count) }
      ...
      <FadeIn>
        <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
          <div>
            ...
            <p className="text-white/70">{t('Snabb, snygg och mobilvänlig.','Fast, pretty and mobile-first.')}</p>
          </div>
      ...
      <FadeIn delay={0.05}>
        <div className="grid md:grid-cols-[1fr_auto_auto] gap-3">
          <input
            ...
            className="card-glass px-4 py-3 text-sm w-full outline-none"
            value={q} onChange={e=> setQ(e.target.value)}
          />
          <select className="card-glass px-3 py-3 text-sm" value={city} onChange={e=> setCity(e.target.value as any)}>
            ...
            <ol className="mt-2 space-y-1 list-decimal pl-5">
              {weekly.map((r,i)=>(
                <li key={r.user} className="text-sm flex justify-between">
                  <span className="text-white/80">#{i+1} {r.user.slice(0,6)}</span>
